...
export function parseBalanceSheet(raw) {
  ...
  function walk(rows = []) {
    for (const row of rows) {
      ...
      if (label) {
        ...
        if (
          /credit cards?|credit card accounts?/i.test(label)
        ) {
          const value = preferSectionValue(
            summary,
            sectionRows
        ...
        if (
          /^total assets less current liabilities$/i.test(label)
        ) {
  ...
  if (!result.totalCurrentLiabilities) {
    result.totalCurrentLiabilities =
  ...
  if (!result.netCurrentAssets) {
    result.netCurrentAssets =
      result.totalCurrentAssets -
      result.totalCurrentLiabilities;
  }

  if (!result.totalAssetsLessCurrentLiabilities) {
    result.totalAssetsLessCurrentLiabilities =
      result.fixedAssets +
      result.netCurrentAssets;
  }
