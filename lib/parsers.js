...
// ── Balance Sheet (✅ FINAL CORRECT VERSION) ───────────────────────────────
...
export function parseBalanceSheet(raw) {
  ...
  function walk(rows = []) {
    for (const row of rows) {
      ...
      // ✅ DEPRECIATION (store total for reference)
      ...
      if (
        ...
        if (!/current assets|total assets/.test(h)) {
          ...
          // ✅ If QBO summary is zero → calculate NBV
          if (!value || value === 0) {

            const gross = extractedRows
              .filter(r => !/depreciation/i.test(r.name))
              .reduce((s, r) => s + r.amount, 0);

            const depreciation = extractedRows
              .filter(r => /depreciation/i.test(r.name))
              .reduce((s, r) => s + r.amount, 0); // negative

            value = gross + depreciation; // ✅ NBV
          }
