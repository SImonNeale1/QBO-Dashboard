...
// ── Balance Sheet (✅ FINAL WORKING VERSION) ───────────────────────────────
...
export function parseBalanceSheet(raw) {
  ...
  // ✅ ✅ ✅ GLOBAL EXTRACTION (THIS IS THE REAL FIX)
  ...
  for (const r of allRows) {
    ...
    // ✅ FIXED MATCHING (no more zero)
    if (name.startsWith('total for') && name.includes('current liabilities')) {
      base = value;
    }

    if (name.startsWith('total for') && name.includes('trade')) {
      trade = value;
    }

    if (name.startsWith('total for') && name.includes('credit card')) {
      cards = value;
    }
  }
