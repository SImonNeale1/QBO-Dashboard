 ...
 * parsers.js
 *
 * QBO report responses are deeply nested andexport function parsePL(raw) { * QBO report responses are deeply nested and inconsistent.
...
export function parseBalanceSheet(raw) {
  ...
  function walk(rows = []) {
    for (const row of rows) {
      ...
      // ✅ ✅ ✅ EXACT MATCH ONLY
      if (h === 'total for current liabilities') {
        result.currentLiabilities = summary;
        console.log('✅ EXACT MATCH FOUND:', header, summary);
      }
  ...
  console.log('Current Liabilities:', result.currentLiabilities);
  ...
  console.log('Total for Current Liabilities:', result.currentLiabilities);
