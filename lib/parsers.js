...
export function parseBalanceSheet(raw) {
  ...
  /*
   * Preserve QuickBooks' reported Total Current Assets.
   *
   * This is used to calculate Other Current Assets as a balancing
   * figure and avoids double-counting nested account rows.
   */
  const reportedTotalCurrentAssets =
    result.totalCurrentAssets;
  ...
  /*
   * Other Current Assets must not be calculated by broadly summing
   * all inventory, stock and prepayment account rows because QBO can
   * include nested rows and intermediate totals.
   *
   * Instead:
   *
   ...
   * - Accounts Receivable
   */
  if (reportedTotalCurrentAssets !== 0) {
    result.currentAssets =
      reportedTotalCurrentAssets -
      result.cash -
      result.accountsReceivable;
  } else if (!result.currentAssets) {
    /*
     * This is only a fallback for reports that do not contain a
     * Total Current Assets section.
     *
     * Exact account labels are used to reduce double-counting.
     */
    result.currentAssets =
      sumMatchingAccounts(
        allAccounts,
        /^inventory$|^stock$|^prepayments?$|^prepaid expenses?$|^other current assets?$/i
      );
  }
  ...
  /*
   * Always calculate final balance-sheet totals from their
