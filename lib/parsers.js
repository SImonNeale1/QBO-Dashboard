...
export function parseBalanceSheet(raw) {
  ...
  function walk(rows = []) {
    for (const row of rows) {
      ...
      // ✅ Get label safely (Header OR Summary)
      const header =
      ...
      // ✅ CURRENT ASSETS (exact enough, no overmatch)
      if (h === 'current assets') {
      ...
      // ✅ ✅ ✅ FIXED ASSETS (BASED ON YOUR ACTUAL DATA)
      if (
      ...
      // ✅ recurse (important for QBO structure)
      if (row.Rows?.Row) {
        walk(row.Rows.Row);
      }
    }
