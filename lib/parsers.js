...
export function parseBalanceSheet(raw) {
  ...
  for (const row of raw.Rows?.Row || []) {
    ...
    // ✅ FIXED ASSETS (TRUST QBO)
      ...
      result.foundFixedAssetsSection = true;

      result.sections.push({
        name: header,
        amount: result.fixedAssets,
        rows
      });
    }

    // ✅ SAFE FALLBACK (ONLY in asset sections + no FA found)
    else if (!result.foundFixedAssetsSection && /assets/i.test(header)) {

      const possibleFA = rows
        .filter(r =>
          /equipment|vehicle|fixture|plant|machinery/i.test(r.name) // tighter match
          && !/depreciation/i.test(r.name) // exclude depreciation
        )
        .reduce((sum, r) => sum + r.amount, 0);

      if (possibleFA > 0) {
        result.fixedAssets = possibleFA;
      }
    }
    ...
    // ✅ LIABILITIES
      ...
      result.sections.push({
        name: header,
        amount: result.currentLiabilities,
        rows
      });
    }
