...
export function parseBalanceSheet(raw) {
  ...
  function walk(rows = []) {
    for (const row of rows) {
      ...
      // ✅ FIXED ASSETS
      if (h === 'non-current assets' || h === 'non current assets') {
        result.fixedAssets = summary;
      }

      // ✅ ✅ ✅ CURRENT LIABILITIES (FINAL CORRECT LOGIC)
      if (h === 'current liabilities') {

        let total = summary;   // ✅ trust QBO first
        let rebuilt = 0;

        for (const sub of row.Rows?.Row || []) {

          const subLabel =
            sub.Header?.ColData?.[0]?.value ||
            sub.Summary?.ColData?.[0]?.value ||
            '';

          const sh = subLabel.toLowerCase();
          const subValue = findSummaryValue(sub);

          // ✅ breakdown
          if (sh.includes('trade creditor')) {
            result.tradeCreditors = subValue;
          }

          if (sh.includes('credit card')) {
            result.creditCards = subValue;
          }

          // ✅ rebuild candidate
          if (
            sh.includes('creditor') ||
            sh.includes('payable') ||
            sh.includes('credit card') ||
            sh.includes('accrual') ||
            sh.includes('tax') ||
            sh.includes('vat') ||
            sh.includes('loan')
        ...
        // ✅ fallback ONLY if summary is missing
        if (!total || total === 0) {
          total = rebuilt;
        }

        result.currentLiabilities = total;
      }
