...
export function parseBalanceSheet(raw) {
  ...
  for (const row of raw.Rows?.Row || []) {
    ...
    // ✅ ✅ ✅ FIXED ASSETS (ROBUST — WILL WORK FOR YOUR DATA)
    if (
      /fixed assets/.test(h) ||
      /non.?current assets/.test(h) ||
      /property/.test(h) ||
      /plant/.test(h) ||
      /equipment/.test(h) ||
      /ppe/.test(h) ||
    ...
    if (/current liabilities/.test(h)) {
      ...
      result.currentLiabilities = liabilities;
      result.accountsPayable = payables;
