...
export function parseBalanceSheet(raw) {
  ...
  // ✅ ✅ ✅ GLOBAL EXTRACTION (THIS FIXES YOUR ISSUE)
  ...
  for (const r of allRows) {
    ...
    if (name === 'total for current liabilities') {
      base = value;
    }

    if (name.includes('total for') && name.includes('trade')) {
      trade = value;
    }

    if (name.includes('total for') && name.includes('credit card')) {
      cards = value;
    }
  }
