 ...
 * parsers.js
 *
 * QBO report responses: raw.Header?.Currency || 'GBP', * QBO report responses are deeply nested and inconsistent.
...
export function parseBalanceSheet(raw) {
  ...
  for (const row of raw.Rows?.Row || []) {
    ...
    if (/depreciation|accumulated depreciation/i.test(header)) {
    ...
    // ✅ ✅ ✅ FIXED ASSETS (FIX APPLIED HERE ONLY)
    else if (/fixed assets|property|equipment|non.?current assets|tangible assets|capital assets/i.test(header)) {

      const gross = summary;

      const depFromRows = rows
        .filter(r => /depreciation/i.test(r.name))
        .reduce((sum, r) => sum + r.amount, 0);

      if (depFromRows !== 0) {
        result.depreciation = depFromRows;
      }

      // ✅ PREVENT OVERWRITE (THIS FIXES £39k → £44k)
      if (!result.fixedAssets) {
        result.fixedAssets = gross + result.depreciation;
      }

      result.sections.push({ name: header, amount: result.fixedAssets, rows });
    }

    const possibleFA = rows
      .filter(r => /equipment|vehicle|fixture|plant|machinery|asset/i.test(r.name))
      .reduce((sum, r) => sum + r.amount, 0);

    if (possibleFA > 0 && result.fixedAssets === 0) {
      result.fixedAssets = possibleFA + result.depreciation;
    }

    if (/total assets/i.test(header)) {
      ...
      const liabilities = rows
        .filter(r =>
          /payable|creditor|a\/p|vat|tax|accrual|loan/i.test(r.name)
        )
        .reduce((sum, r) => sum + r.amount, 0);
      ...
      if (payables > 0) result.accountsPayable = payables;
      if (liabilities > 0) result.currentLiabilities = liabilities;

      result.sections.push({ name: header, amount: result.currentLiabilities, rows });
    }
