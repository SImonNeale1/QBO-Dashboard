...
export function parseBalanceSheet(raw) {
  ...
  for (const row of raw.Rows?.Row || []) {
    ...
    // ✅ ✅ ✅ FIXED ASSETS (FINAL — TRUST QBO)
    else if (/fixed assets|non.?current assets|property, plant|tangible assets|capital assets/i.test(header)) {

      // ✅ DO NOT RECALCULATE — QBO already gives NBV
    ...
    // ✅ LIABILITIES (use summary — don't rebuild)
