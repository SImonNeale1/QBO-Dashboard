...
export function parseBalanceSheet(raw) {
  ...
  for (const row of raw.Rows?.Row || []) {
    ...
    // ✅ FIXED ASSETS (FINAL CORRECT MATCH)
    else if (/fixed assets|non.?current assets|tangible assets|capital assets|property.*plant.*equipment|property.*equipment|ppe/i.test(header)) {
