...
export function parseBalanceSheet(raw) {
  ...
  function walk(rows = []) {
    for (const row of rows) {
      ...
      // ✅ ✅ ✅ KEY FIX: capture summary for Current Liabilities
      if (h.includes('current liabilities') && summary) {
        result.currentLiabilities = summary;
        console.log('✅ FOUND TOTAL CURRENT LIABILITIES (SUMMARY):', header, summary);
      }
  ...
  // ✅ FINAL RULE: use summary value only
  // (fallback only if summary somehow missing)
