/**
 * parsersBO report responses are deeply nested and inconsistent. * parsers.js
 */
...
// ── Balance Sheet (✅ USER-SPECIFIC LOGIC APPLIED) ─────────────────────────
...
export function parseBalanceSheet(raw) {
  ...
  function walk(rows = []) {
    for (const row of rows) {
      ...
      // ✅ ✅ ✅ CURRENT LIABILITIES (YOUR REQUIRED LOGIC)
      if (
        h === 'total for current liabilities' ||
        ...
        for (const r of rowsData) {
          ...
          if (name === 'total for trade creditors') {
            trade = value;
          }

          if (name === 'total for credit cards') {
            cards = value;
          }
        }
