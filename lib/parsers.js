...
export function parseBalanceSheet(raw) {
  ...
  function walk(rows = []) {
    for (const row of rows) {
      const label = normaliseLabel(
        getRowHeader(row) || getSummaryLabel(row)
      );
      ...
      if (label) {
        if (isFixedAssetsLabel(label)) {
          result.fixedAssets = preferSectionValue(summary, sectionRows);
        ...
        if (
          /cash and cash equivalents|^cash$|^bank$|bank accounts?/i.test(
            label
          )
        ) {
          const value = preferSectionValue(summary, sectionRows);
        ...
        if (isCurrentLiabilitiesLabel(label)) {
        ...
        if (
          /^total liabilities$|^liabilities total$/i.test(label)
        ) {
  ...
  if (!result.currentAssets) {
    const otherCurrentAssets = sumMatchingAccounts(
      allAccounts,
  ...
  // Cash and receivables are components of current assets,
  ...
  // Accounts payable is a component of current liabilities,
