...
export function parsePL(raw) {
  ...
  for (const row of raw.Rows?.Row || []) {
    ...
    if (/income|revenue/i.test(header)) {
      ...
      result.costOfSales = summary;
      result.sections.push({ name: header, amount: summary, rows: extractRows(row) });
    } else if (/gross profit/i.test(header)) {
      result.grossProfit = summary;
    } else if (/expenses?/i.test(header)) {
      result.expenses = summary;
      result.sections.push({ name: header, amount: summary, rows: extractRows(row) });
    } else if (/net (income|earnings|profit|loss)/i.test(header)) {
      result.netIncome = summary;
    }
  }

  // Derive gross profit if QBO didn't return it explicitly
  ...
  result.grossMarginPct = result.revenue !== 0
    ? round(result.grossProfit / result.revenue * 100, 1) : 0;
  result.netMarginPct = result.revenue !== 0
    ? round(result.netIncome / result.revenue * 100, 1) : 0;
...
export function parseBalanceSheet(raw) {
  ...
  for (const row of raw.Rows?.Row || []) {
    ...
    if (/current assets/i.test(header)) {
      ...
      result.cash = findLineValue(rows, /cash|bank/i);
