...
export function parseBalanceSheet(raw) {
  ...
  function walk(rows = []) {
    for (const row of rows) {
      ...
      // ✅ ✅ ✅ CURRENT LIABILITIES (SIMPLE + CORRECT)
      if (h.includes('current liabilities')) {
        ...
        for (const r of rowsData) {
          ...
          if (name.includes('total') && name.includes('trade')) {
            result.tradeCreditors = value;
          }

          if (name.includes('total') && name.includes('credit card')) {
